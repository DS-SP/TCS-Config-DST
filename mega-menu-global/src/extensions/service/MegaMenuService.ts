...
export class MegaMenuService {
  public static readonly useSampleData: boolean = false;

  public static readonly level1ListName: string = "Mega Menu - Level 1";
  public static readonly level2ListName: string = "Mega Menu - Level 2";
  public static readonly level3ListName: string = "Mega Menu - Level 3";

  public static readonly cacheKey: string = "MegaMenuTopLevelItems";

  // Get items for the menu and cache the result in session cache.
  public static getMenuItems(
    siteCollectionUrl: string
  ): Promise<TopLevelMenu[]> {
    if (!MegaMenuService.useSampleData) {
      return new Promise<TopLevelMenu[]>((resolve, reject) => {
        // See if we've cached the result previously.
        var topLevelItems: TopLevelMenu[] = pnp.storage.session.get(
          MegaMenuService.cacheKey
        );
        ...
        if (topLevelItems) {
          ...
          console.log(
            "Didn't find mega menu items in cache, getting from list."
          );

          var level1ItemsPromise = MegaMenuService.getMenuItemsFromSp(
            MegaMenuService.level1ListName,
            siteCollectionUrl
          );
          var level2ItemsPromise = MegaMenuService.getMenuItemsFromSp(
            MegaMenuService.level2ListName,
            siteCollectionUrl
          );
          var level3ItemsPromise = MegaMenuService.getMenuItemsFromSp(
            MegaMenuService.level3ListName,
            siteCollectionUrl
          );
          ...
          Promise.all([
            ...
            topLevelItems = MegaMenuService.convertItemsFromSp(
              results[0],
              results[1],
              results[2]
            );
